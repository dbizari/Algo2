...
int interpret_bf(vector_t * tape){
	...
	char * array_char = calloc(SIZE_INIT_MEM,sizeof(char));
	if(!array_char) return EXIT_FAILURE;
	char * ptr = array_char;

	//mem_size = SIZE_INIT_MEM;
    for (i = 0; (current_char = *(char *)vector_obtener(tape,i)) != '\0'; i++){
		//printf("%c\t%lu\n",current_char,i );
        if (current_char == '>') {
			++ptr;
			/*++mem_pos
			if(mem_pos == (mem_size - 1)){
				array_char = realloc(array_char, SCALE_FACTOR * mem_size * sizeof(char));
				if(!aux){
					status = false;
					break;
				}
